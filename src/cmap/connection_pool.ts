...
export class ConnectionPool extends TypedEventEmitter<ConnectionPoolEvents> {
  ...
  ready(): void {
    ...
    this.emit(
      ...
      new ConnectionPoolReadyEvent(this),
    );
  ...
  private createConnection(callback: Callback<Connection>) {
    ...
    connect(connectOptions, (err, connection) => {
      if (err || !connection) {
        ...
        callback(
          err ??
            new MongoRuntimeError('Connection creation failed without error'),
        );
        return;
      }
      ...
      if (this[kPoolState] !== PoolState.ready) {
        ...
        callback(
          this.closed ? new PoolClosedError(this) : new PoolClearedError(this),
        );
        return;
      }
      ...
      console.log(738);
      this.emit('availableConnections', this.availableConnectionCount);
    ...
    while (this.waitQueueSize) {
      ...
      if (this[kPoolState] !== PoolState.ready) {
        ...
        const error = this.closed
          ? new PoolClosedError(this)
          : new PoolClearedError(this);
        this.emit(
          ConnectionPool.CONNECTION_CHECK_OUT_FAILED,
